...
echo "=================================================="
echo "Unwrapping Bridged Token (Gonka -> Ethereum)"
echo "=================================================="
...
if [ -z "$CW20_ADDR" ] || [ -z "$AMOUNT" ] || [ -z "$DESTINATION" ] || [ -z "$BRIDGE_ADDR" ]; then
    echo "Usage: ./bridge-token-unwrap.sh --cw20 <ADDR> --amount <AMT> --destination <ETH_ADDR> --bridge <BRIDGE_ADDR> [--key-string <STR>] [--local]"
    exit 1
fi
...
EXEC_MSG=$(jq -n --arg amt "$AMOUNT" --arg dest "$DESTINATION" --arg bridge "$BRIDGE_ADDR" \
  '{withdraw: {amount: $amt, destination_bridge_address: $bridge, destination_address: $dest}}')

echo "Executing unwrap (withdraw) on CW20 $CW20_ADDR..."
printf "%s\n%s\n" "$PASSWORD" "$PASSWORD" | $APP_NAME tx wasm execute "$CW20_ADDR" "$EXEC_MSG" \
